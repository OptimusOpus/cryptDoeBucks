...
function captureConsoleOutput(action: () => Promise<void>): Promise<string[]> {
  return new Promise(async resolve => {
    ...
    console.log = (message: any, ...optionalParams: any[]) => {
      ...
      output.push(
        stringMessage + (optionalParams.length > 0 ? ' ' + optionalParams.join(' ') : ''),
      );
    };
    await action();
...
describe('Local Deployment Script (scripts/deploy.ts --network hardhat)', function () {
  ...
  it('VRFv2Consumer should be linked to VRFCoordinatorV2Mock', async function () {
    const vrfConsumer = await ethers.getContractAt(
      'VRFv2Consumer',
      deployedContractAddresses['VRFv2Consumer'],
    );
    ...
    expect(coordinatorAddressInConsumer).to.equal(
      deployedContractAddresses['VRFCoordinatorV2Mock'],
    );
    ...
    const isConsumerAdded = await vrfCoordinatorMock.consumerIsAdded(
      BigInt(subIdFromLog!),
      deployedContractAddresses['VRFv2Consumer'],
    );
    expect(isConsumerAdded, "VRFv2Consumer was not added to the mock coordinator's subscription").to
      .be.true;
  });
  ...
  it('PrizePool should have CrypdoeBucks contract address set', async function () {
    const prizePool = await ethers.getContractAt(
      'PrizePool',
      deployedContractAddresses['PrizePool'],
    );
  ...
  it('CrypdoeBucks should have FightLib linked and VRF/PrizePool addresses set', async function () {
    const crypdoeBucks = await ethers.getContractAt(
      'CrypdoeBucks',
      deployedContractAddresses['CrypdoeBucks'],
    );
    ...
    expect(true, 'FightLib linking is implicitly tested by successful CrypdoeBucks deployment').to
      .be.true;
  });
});
