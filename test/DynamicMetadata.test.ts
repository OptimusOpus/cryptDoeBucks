...
import { CrypdoeBucks, MetadataLib, PrizePool, VRFv2Consumer, VRFCoordinatorV2Mock } from "../typechain-types";
...
describe("Dynamic Metadata", function () {
  ...
  describe("Basic Metadata Generation", function () {
    ...
    it("Should include correct attributes in metadata", async function () {
      ...
      const doesAttr = metadata.attributes.find((attr: any) => attr.trait_type === "Does Controlled");
  ...
  describe("Dynamic Updates", function () {
    ...
    it("Should update metadata after training", async function () {
      ...
      let initialValue = initialMetadata.attributes.find((attr: any) => attr.trait_type === "Strength").value;
      
      if (initialValue >= 10) {
        traitToTrain = 1; // Speed
        initialValue = initialMetadata.attributes.find((attr: any) => attr.trait_type === "Speed").value;
      }
      if (initialValue >= 10) {
        traitToTrain = 2; // Vitality
        initialValue = initialMetadata.attributes.find((attr: any) => attr.trait_type === "Vitality").value;
      }
      if (initialValue >= 10) {
        traitToTrain = 3; // Intelligence
        initialValue = initialMetadata.attributes.find((attr: any) => attr.trait_type === "Intelligence").value;
      }
      ...
      // Train the buck
      await cryptDoeBucks.connect(user1).trainBuck(0, traitToTrain, { value: ethers.parseEther("0.01") });
      ...
      const updatedValue = updatedMetadata.attributes.find((attr: any) => attr.trait_type === traitNames[traitToTrain]).value;
    ...
    it("Should emit MetadataUpdate event when training", async function () {
      ...
      const strengthValue = initialMetadata.attributes.find((attr: any) => attr.trait_type === "Strength").value;
      
      if (strengthValue < 10) {
        await expect(
          cryptDoeBucks.connect(user1).trainBuck(0, 0, { value: ethers.parseEther("0.01") })
        ).to.emit(cryptDoeBucks, "MetadataUpdate").withArgs(0);
      } else {
