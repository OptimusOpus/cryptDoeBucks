...
// Types are used implicitly through ethers.getContractFactory
...
describe('Workshop Unit Tests', function () {
  ...
  describe('#addBar', function () {
    ...
    it('Should allow multiple users to add bars', async function () {
      ...
      await workshop
        .connect(user1)
        .addBar('User1 Bar', true, [1, 2, 3, 4] as [number, number, number, number]);

      // User 2 adds a bar
      await workshop
        .connect(user2)
        .addBar('User2 Bar', false, [5, 6, 7, 8] as [number, number, number, number]);
  ...
  describe('#getBars', function () {
    ...
    it('Should return all bars when they exist', async function () {
      ...
      await workshop
        .connect(user1)
        .addBar('Bar 1', true, [1, 2, 3, 4] as [number, number, number, number]);
      await workshop
        .connect(user1)
        .addBar('Bar 2', false, [5, 6, 7, 8] as [number, number, number, number]);
  ...
  describe('#getBarFoos', function () {
    it('Should return the foos for a specific bar', async function () {
      ...
      await workshop
        .connect(user1)
        .addBar('Test Bar', true, [1, 2, 3, 4] as [number, number, number, number]);
  ...
  describe('Edge Cases', function () {
    it('Should handle name at minimum length', async function () {
      ...
      await expect(
        workshop.addBar(name, true, [1, 2, 3, 4] as [number, number, number, number]),
      ).to.emit(workshop, 'BarCreated');
    });
    ...
    it('Should handle many bars being added', async function () {
      ...
      for (let i = 0; i < 5; i++) {
        await workshop
          .connect(user1)
          .addBar(`Bar ${i}`, i % 2 === 0, [i, i + 1, i + 2, i + 3] as [
            number,
            number,
            number,
            number,
